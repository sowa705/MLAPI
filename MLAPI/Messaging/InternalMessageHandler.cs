    ...
    internal static class InternalMessageHandler
    {
        ...
        internal static void HandleRemoveClient(ulong clientId, Stream stream)
        {
            using (PooledBitReader reader = PooledBitReader.Get(stream))
            {
                ...
                for (int i = 0; i < HostMigrationManager.MigratableClients.Count; i++)
                {
                    if (HostMigrationManager.MigratableClients[i].ClientId == clientIdToRemove)
                    {
                        HostMigrationManager.MigratableClients.RemoveAt(i);
                        break;
                    }
                }
            }
